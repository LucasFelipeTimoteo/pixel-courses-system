...
export const editUserHandler = async (req: EditUserHandlerRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const updatedUser = await usersRepositoryMongoose.editUser(userId, validatedUserEdition)
    ...
    return res.status(200).json({ message: `Successfully update user ${updatedUser._id}` });
  }
  catch (error) {
