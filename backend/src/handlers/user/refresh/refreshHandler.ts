...
export const refreshHandler = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const validatedrefreshToken = jwt.verify(refreshToken, appEnv.REFRESH_TOKEN_JWT_SECRET) as UserToken
    ...
    const newToken = jwt.sign(newTokenPayload, appEnv.ACCESS_TOKEN_JWT_SECRET, { expiresIn: accessTokenTTL })
