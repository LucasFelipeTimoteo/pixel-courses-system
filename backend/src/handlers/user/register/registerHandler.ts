...
export const registerHandler = async (req: RegisterHandlerRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const accessToken = jwt.sign(tokenPayload, appEnv.ACCESS_TOKEN_JWT_SECRET, { expiresIn: accessTokenTTLInSeconds })
    const refreshToken = jwt.sign(tokenPayload, appEnv.REFRESH_TOKEN_JWT_SECRET, { expiresIn: refreshTokenTTLInSeconds })
